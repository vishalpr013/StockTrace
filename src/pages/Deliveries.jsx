...
export const Deliveries = () => {
  ...
  const loadData = async () => {
    const [docs, whs, locs, prods] = await Promise.all([
      getDocuments('deliveries', statusFilter),
      getWarehouses(),
      getLocations(),
      getProducts()
    ]);
  ...
  const handleCreate = () => {
    ...
    setFormData({
      ...
      warehouse_id: '',
      customer_name: '',
      lines: [{ product_id: '', from_location_id: '', quantity: 0 }]
    });
    setShowModal(true);
  };

  const handleEdit = async (doc) => {
    const fullDoc = await api.getDocument('deliveries', doc.id);
    setEditingDoc(fullDoc);
    setFormData({
      date: fullDoc.date,
      warehouse_id: fullDoc.warehouse_id,
      customer_name: fullDoc.customer_name || '',
      lines: fullDoc.lines.map(l => ({ product_id: l.product_id, from_location_id: l.from_location_id, quantity: l.quantity }))
    });
    setShowModal(true);
  };

  const addLine = () => {
    setFormData({ ...formData, lines: [...formData.lines, { product_id: '', from_location_id: '', quantity: 0 }] });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingDoc) {
        const updated = await api.updateDocument('deliveries', editingDoc.id, formData);
        updateDocument('deliveries', editingDoc.id, updated);
        setDocuments(prev => prev.map(d => d.id === editingDoc.id ? updated : d));
      } else {
        const newDoc = await api.createDocument('deliveries', formData);
        addDocument('deliveries', newDoc);
  ...
  const handleConfirm = async (docId) => {
    if (!confirm('Confirm this delivery? This will update stock levels.')) return;
    try {
      await api.confirmDocument('deliveries', docId);
      invalidateCache('deliveries');
      loadData();
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage outgoing stock deliveries</p>
        </div>
          ...
          <tbody>
            {documents.map((doc) => (
              <tr key={doc.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="py-3 px-4">{doc.customer_name}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
          <div className="bg-white rounded-lg p-6 w-full max-w-3xl m-4">
            <h2 className="text-xl font-bold mb-4">{editingDoc ? 'Edit Delivery' : 'New Delivery'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={formData.customer_name} onChange={(e) => setFormData({ ...formData, customer_name: e.target.value })} required className="w-full px-3 py-2 border border-gray-300 rounded-lg" />
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {formData.lines.map((line, index) => (
                    <div key={index} className="grid grid-cols-12 gap-2 items-center">
                      ...
                      <select value={line.from_location_id} onChange={(e) => updateLine(index, 'from_location_id', e.target.value)} required className="col-span-4 px-3 py-2 border border-gray-300 rounded-lg text-sm">
